import React from "react";
import "../AboutUs/aboutus.css";
const aboutus = () => {
  return (
    <>
      <section id="aboutus" className="about-section">
        <div className="about-poster">
          <h1 className="about-poster-head">
            Innovating for a Connected Future with Purposeful Solutions and
            Exceptional Service
          </h1>
          <p className="about-poster-para">
            At Direccion, we're driven by a passion for innovation and a
            commitment to excellence. With a focus on connectivity and
            purposeful solutions, we strive to shape the future by harnessing
            the power of technology. Our dedicated team works tirelessly to
            create products and services that not only meet but exceed the
            evolving needs of our customers. From cutting-edge advancements to
            personalized support, we're here to empower individuals and
            businesses alike on their journey towards success. Join us as we
            pave the way for a brighter, more connected tomorrow
          </p>
        </div>
        <div className="custom-shape-divider-bottom-1713428901">
          <svg
            data-name="Layer 1"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 1200 120"
            preserveAspectRatio="none"
          >
            <path
              d="M649.97 0L599.91 54.12 550.03 0 0 0 0 120 1200 120 1200 0 649.97 0z"
              className="shape-fill"
            />
          </svg>
        </div>
        <div className="about-black-poster">
          <h1 className="about-black-poster-head text-center">
            Crafting Innovative Solutions for a Connected World with{" "}
            <span className="about-black-poster-span">The Direccion</span>
          </h1>
        </div>

        <div id="purpose" className="about-main container">
          <div className="container about-container">
            <div className="left-div-about">
              <h2 className="left-head-text-about">
                Fostering Impactful Change Through Innovation and Dedication
                with{" "}
                <span className="left-head-text-span-about">Direccion</span>
              </h2>

              <p className="left-head-para-about">
                At Direccion, our expertise lies at the intersection of AI,
                development, and consulting solutions. Leveraging cutting-edge
                technologies and deep industry insights, we empower businesses
                to thrive in the digital age. From AI-driven automation to
                bespoke development projects, our solutions are tailored to meet
                the unique needs of each client. Backed by a team of seasoned
                professionals, we provide comprehensive consulting services that
                drive innovation and foster growth. Whether you're navigating
                the complexities of AI integration or seeking strategic
                development guidance, Direccion is your trusted partner every
                step of the way
              </p>
              <button className="ai-button-about">Lets Talk</button>
            </div>
            <div className="right-div-about">
              <img
                src="purpose.webp"
                alt="AI Image"
                className="image-ai-about"
              />
            </div>
          </div>
        </div>
      </section>

      <section id="value" className="value-section">
        <div className="value-main container">
          <div className="container value-container">
            <div className="right-div-value">
              <img src="value.jpg" alt="AI Image" className="image-value" />
            </div>
            <div className="left-div-value">
              <h2 className="left-head-text-value">
                Our Core Principles Define the{" "}
                <span className="left-head-text-span-value">Direccion</span>{" "}
                Difference
              </h2>

              <p className="left-head-para-value">
                At Direccion, our values serve as the bedrock of everything we
                do. We are committed to fostering growth, driving innovation,
                and delivering excellence in every endeavor. Integrity,
                transparency, and collaboration are at the heart of our
                operations, ensuring that we build lasting relationships based
                on trust and mutual respect. Our dedication to continuous
                improvement drives us to push boundaries, embrace challenges,
                and seize opportunities for innovation. By staying true to our
                values, we create value for our clients, our team, and the
                communities we serve, setting the standard for excellence in our
                industry
              </p>
              <button className="value-button">Connect</button>
            </div>
          </div>
        </div>
      </section>

      <section id="team" className="team-section">
        <div className="team-main container">
          <div className="container team-container">
            <div className="left-div-team">
              <h2 className="left-head-text-team">
                Discover the Talent Behind{" "}
                <span className="left-head-text-span-team">Direccion:</span> Our
                Dedicated Team Fueling Success
              </h2>

              <p className="left-head-para-team">
                At Direccion, our team members are the heart and soul of our
                company. Comprised of talented individuals from diverse
                backgrounds, each member brings a unique set of skills,
                perspectives, and experiences to the table. Together, we foster
                a culture of collaboration, creativity, and continuous learning,
                empowering each other to reach new heights of success. Our
                team's unwavering dedication and passion drive our mission
                forward, enabling us to deliver innovative solutions and exceed
                the expectations of our clients. With a shared commitment to
                excellence, we celebrate our collective achievements and embrace
                the challenges ahead, confident in our ability to overcome them
                as one cohesive unit
              </p>
              <button className="ai-button-team">Contact us</button>
            </div>
            <div className="right-div-team">
              <img src="team.webp" alt="AI Image" className="image-ai-team" />
            </div>
          </div>
        </div>
      </section>
      <div className="shape-divider">
        <div className="cube" />

        <div className="cube" />
      </div>

      <section id="why" className="why-section">
        <div className="why-main container">
          <div className="container why-container">
            <div className="right-div-why">
              <img src="why.avif" alt="AI Image" className="image-why" />
            </div>
            <div className="left-div-why">
              <h2 className="left-head-text-why">
                Why{" "}
                <span className="left-head-text-span-why">
                  Choose Direccion?
                </span>{" "}
                Discover Our Commitment to Excellence
              </h2>

              <p className="left-head-para-why">
                At Direccion, we pride ourselves on our unwavering commitment to
                excellence, evident in every aspect of our operations. Our
                success is not just measured by achieving goals but by the
                quality of those achievements. We prioritize delivering
                top-notch services and solutions that surpass expectations,
                driven by a dedication to innovation and continuous improvement.
                Moreover, our robust support system ensures that clients receive
                the guidance and assistance they need every step of the way.
                With our team of seasoned experts, we provide the best
                consultancy services tailored to meet the specific needs and
                challenges of each client. At Direccion, excellence is not just
                a goal—it's our standard
              </p>
              <button className="why-button">Connect</button>
            </div>
          </div>
        </div>
        <div className="custom-shape-divider-bottom-1713435677">
          <svg
            data-name="Layer 1"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 1200 120"
            preserveAspectRatio="none"
          >
            <path
              d="M598.97 114.72L0 0 0 120 1200 120 1200 0 598.97 114.72z"
              className="shape-fill"
            />
          </svg>
        </div>
      </section>
    </>
  );
};

export default aboutus;
